...
import re
import secrets
from datetime import datetime, timedelta        #проверить работоспособность отправки письма
...
def is_valid_email(email):
    """Проверка корректности email"""  #эачем везде эти коментарии их лучше удалить или вынести хештегом
...
def recovery():
    ...
    if email in users_db:
        ...
        print(f"\n=== Ссылка для сброса пароля для {email} ===")                                    #зачем эти принты если они работают то из лучше удалить тк пользователь их всё равно не видит
        print(f"Имя пользователя: {users_db[email]['name']}")
        print(f"Ссылка: {reset_link}")
        print(f"Срок действия: 24 часа\n")

        flash('Инструкции по восстановлению пароля отправлены на ваш email', 'success')         #зачем это не будет высвечиваться тк мы его перенаправляем лучше удалить
